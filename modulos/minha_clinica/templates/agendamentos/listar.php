...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-status', function(e) {
        ...
        if (!confirm('Deseja ' + (labelStatus[status] || status) + ' este agendamento #' + id + '?')) {
            return;
        }

        $.get('index.php?module=minha_clinica&action=api&api_action=alterar_status&id=' + id + '&status=' + status, function(response) {
            if (response.success) {
                // Atualizar badge de status na tabela
                var statusClass = {
                    'agendado': 'warning', 'confirmado': 'info', 'em_atendimento': 'primary',
                    'realizado': 'success', 'cancelado': 'danger', 'faltou': 'secondary'
                };
                var badgeClass = statusClass[status] || 'secondary';
                var badgeLabel = status.charAt(0).toUpperCase() + status.slice(1).replace('_', ' ');
                tr.find('td:eq(7) .badge').attr('class', 'badge badge-' + badgeClass).text(badgeLabel);

                // Feedback visual
                tr.css('background-color', '#d4edda').delay(1000).queue(function(next) {
                    $(this).css('background-color', '');
                    next();
                });
            } else {
                alert('Erro: ' + (response.message || 'Falha ao alterar status'));
            }
        }, 'json').fail(function() {
