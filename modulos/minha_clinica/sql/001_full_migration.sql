...
CREATE TABLE IF NOT EXISTS master_guias (
    ...
    procedimento_id INT NULL, -- Procedimento principal da guia (simplificado para MVP)
    ...
    updated_at TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
     FOREIGN KEY (convenio_id) REFERENCES master_convenios(id),
     FOREIGN KEY (procedimento_id) REFERENCES master_procedimentos(id), -- Add this if guides are per procedure or link via items table
...
ALTER TABLE master_agendamentos ADD COLUMN IF NOT EXISTS convenio_id INT NULL;
ALTER TABLE master_agendamentos ADD COLUMN IF NOT EXISTS guia_id INT NULL;
...
ALTER TABLE master_procedimentos ADD COLUMN IF NOT EXISTS codigo_padrao VARCHAR(50);

-- Atualização de Status de Agendamento
-- Garante que o ENUM tenha os status necessários
ALTER TABLE master_agendamentos MODIFY COLUMN status ENUM('agendado', 'confirmado', 'cancelado', 'realizado', 'falta') DEFAULT 'agendado';

-- Inserção de Permissões (Específico Minha Clínica)
...
('Visualizar Prontuário', 'minha_clinica_pacientes', 'Permite visualizar prontuários de pacientes da clínica'),
('Imprimir Evolução', 'imprimir_evolucao', 'Permite imprimir evoluções clínicas');

-- Associar ao Admin (ID 1)
INSERT IGNORE INTO `perfil_permissoes` (`perfil_id`, `permissao_id`)
SELECT 1, id FROM `permissoes` WHERE `chave` IN ('painel_profissional', 'minha_clinica_pacientes', 'imprimir_evolucao');
