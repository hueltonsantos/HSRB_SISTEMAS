...
class ClinicaModel extends Model {
    ...
    /**
 * Adiciona uma especialidade à clínica
 * 
 * @param int $clinicaId ID da clínica
 ...
 */
public function adicionarEspecialidade($clinicaId, $especialidadeId) {
    $clinicaId = (int)$clinicaId;
    $especialidadeId = (int)$especialidadeId;
    
    // Verifica se a relação já existe
    $sql = "SELECT COUNT(*) as total FROM especialidades_clinicas 
            WHERE clinica_id = ? AND especialidade_id = ?";
    $result = $this->db->fetchOne($sql, [$clinicaId, $especialidadeId]);
...
/**
 * Remove uma especialidade da clínica
 * 
 * @param int $clinicaId ID da clínica
 * @param int $especialidadeId ID da especialidade
 * @return bool Retorna true se a operação foi bem-sucedida, false caso contrário
 */
public function removerEspecialidade($clinicaId, $especialidadeId) {
    $clinicaId = (int)$clinicaId;
    $especialidadeId = (int)$especialidadeId;
    
    $sql = "DELETE FROM especialidades_clinicas 
            WHERE clinica_id = ? AND especialidade_id = ?";
    
    try {
        $this->db->query($sql, [$clinicaId, $especialidadeId]);
        return true;
    } catch (Exception $e) {
        return false;
    }
